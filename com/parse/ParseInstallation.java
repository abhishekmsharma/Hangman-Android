...
public class ParseInstallation extends ParseObject {
    ...
    private static final List<String> READ_ONLY_FIELDS = Collections.unmodifiableList(Arrays.asList(new String[]{KEY_DEVICE_TYPE, KEY_INSTALLATION_ID, KEY_DEVICE_TOKEN, KEY_PUSH_TYPE, KEY_TIME_ZONE, KEY_LOCALE, KEY_APP_VERSION, KEY_APP_NAME, KEY_PARSE_VERSION, KEY_APP_IDENTIFIER}));
    private static final String TAG = "com.parse.ParseInstallation";

    class C02672 implements Continuation<Void, Task<Void>> {
        C02672() {
        }
    ...
    class C02683 implements Continuation<Void, Task<Void>> {
        C02683() {
        }
    ...
    Task<Void> handleSaveResultAsync(State result, ParseOperationSet operationsBeforeSave) {
        ...
        return result == null ? task : task.onSuccessTask(new C02672());
    }

    Task<Void> handleFetchResultAsync(State newState) {
        return super.handleFetchResultAsync(newState).onSuccessTask(new C02683());
    }
    ...
    private void updateVersionInfo() {
        synchronized (this.mutex) {
            try {
                ...
                PLog.m8w(TAG, "Cannot load package info; will not be saved to installation");
            }
