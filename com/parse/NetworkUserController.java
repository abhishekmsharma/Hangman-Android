...
class NetworkUserController implements ParseUserController {
    ...
    class C01681 implements Continuation<JSONObject, State> {
        C01681() {
        }
    ...
    class C01692 implements Continuation<JSONObject, State> {
        C01692() {
        }
    ...
    class C01725 implements Continuation<JSONObject, State> {
        C01725() {
        }
    ...
    public Task<State> signUpAsync(State state, ParseOperationSet operations, String sessionToken) {
        return ParseRESTUserCommand.signUpUserCommand(this.coder.encode(state, operations, PointerEncoder.get()), sessionToken, this.revocableSession).executeAsync(this.client).onSuccess(new C01681());
    }

    public Task<State> logInAsync(String username, String password) {
        return ParseRESTUserCommand.logInUserCommand(username, password, this.revocableSession).executeAsync(this.client).onSuccess(new C01692());
    }
    ...
    public Task<State> getUserAsync(String sessionToken) {
        return ParseRESTUserCommand.getCurrentUserCommand(sessionToken).executeAsync(this.client).onSuccess(new C01725());
    }
