        ...
        synchronized (this.lock) {
            if (this.installationId == null) {
                try {
                    ...
                    PLog.m4i(TAG, "Couldn't find existing installationId file. Creating one instead.");
                } catch (IOException e2) {
                    PLog.m3e(TAG, "Unexpected exception reading installation id from disk", e2);
                }
            }
            if (this.installationId == null) {
    ...
    private void setInternal(String newInstallationId) {
        synchronized (this.lock) {
            try {
                ...
                PLog.m3e(TAG, "Unexpected exception writing installation id to disk", e);
            }
