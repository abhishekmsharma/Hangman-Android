...
public class ParseFile {
    ...
    class C02523 implements Continuation<Void, Task<Void>> {
        C02523() {
        }
    ...
    public ParseFile(String name, byte[] data, String contentType) {
        ...
        if (data.length > MAX_FILE_SIZE) {
            throw new IllegalArgumentException(String.format("ParseFile must be less than %d bytes", new Object[]{Integer.valueOf(MAX_FILE_SIZE)}));
        } else {
            this.data = data;
        }
    }
    ...
    private Task<Void> setPinnedInBackground(final boolean pinned) {
        return this.taskQueue.enqueue(new C02523()).continueWith(new Continuation<Void, Void>() {
    ...
    private Task<Void> saveAsync(final String sessionToken, final ProgressCallback uploadProgressCallback, Task<Void> toAwait, final Task<Void> cancellationToken) {
        ...
        if (cancellationToken == null || !cancellationToken.isCancelled()) {
            return toAwait.continueWithTask(new Continuation<Void, Task<Void>>() {
                ...
                class C02531 implements Continuation<State, Task<Void>> {
                    C02531() {
                    }
                ...
                public Task<Void> then(Task<Void> task) throws Exception {
                    ...
                    if (cancellationToken == null || !cancellationToken.isCancelled()) {
                        return ParseFile.getFileController().saveAsync(ParseFile.this.state, ParseFile.this.data, sessionToken, ParseFile.progressCallbackOnMainThread(uploadProgressCallback), cancellationToken).onSuccessTask(new C02531());
                    }
    ...
    private Task<byte[]> getDataAsync(final ProgressCallback progressCallback, Task<Void> toAwait, final Task<Void> cancellationToken) {
        ...
        if (cancellationToken == null || !cancellationToken.isCancelled()) {
            return toAwait.continueWithTask(new Continuation<Void, Task<byte[]>>() {
                ...
                class C02581 implements Continuation<File, byte[]> {
                    C02581() {
                    }
                ...
                public Task<byte[]> then(Task<Void> task) throws Exception {
                    ...
                    if (cancellationToken == null || !cancellationToken.isCancelled()) {
                        return ParseFile.getFileController().fetchAsync(ParseFile.this.state, null, ParseFile.progressCallbackOnMainThread(progressCallback), cancellationToken).onSuccess(new C02581());
                    }
                    return Task.cancelled();
                }
            });
    ...
    ParseFile(JSONObject json, ParseDecoder decoder) {
        this(new Builder().name(json.optString("name")).url(json.optString("url")).build());
    }
