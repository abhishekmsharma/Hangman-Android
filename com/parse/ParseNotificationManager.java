...
class ParseNotificationManager {
    ...
    public Notification createNotification(Context context, String title, String body, Class<? extends Activity> clazz, int iconId, Bundle extras) {
        if (!isValidIconId(context, iconId)) {
            PLog.m2e(TAG, "Icon id " + iconId + " is not a valid drawable. Trying to fall back to " + "default app icon.");
            iconId = ManifestInfo.getIconId();
        }
        if (iconId == 0) {
            PLog.m2e(TAG, "Could not find a valid icon id for this app. This is required to create a Notification object to show in the status bar. Make sure that the <application> in in your Manifest.xml has a valid android:icon attribute.");
            return null;
        } else if (context == null || title == null || body == null || clazz == null || iconId == 0) {
            PLog.m2e(TAG, "Must specify non-null context, title, body, and activity class to show notification.");
            return null;
        } else {
