...
class ParseKeyValueCache {
    ...
    static int maxKeyValueCacheFiles = DEFAULT_MAX_KEY_VALUE_CACHE_FILES;

    static class C00912 implements Comparator<File> {
        C00912() {
        }
            ...
            if (file == null) {
                ...
                if (getKeyValueCacheAge(file) < Math.max(0, now.getTime() - maxAgeMilliseconds)) {
                    ...
                    try {
                        ...
                        PLog.m3e("ParseKeyValueCache", "error reading from cache", e);
                        str = null;
                    }
                }
            }
    ...
    static JSONObject jsonFromKeyValueCache(String key, long maxAgeMilliseconds) {
        ...
        try {
            ...
            PLog.m3e("ParseKeyValueCache", "corrupted cache for " + key, e);
